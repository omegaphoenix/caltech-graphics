...
static pair<int, int> get_edge_key(int x, int y);
static void hash_edge(map<pair<int, int>, HE*> &edge_hash,
...
static void delete_HE(vector<HEV*> *hevs, vector<HEF*> *hefs);

static Eigen::Vector3d HEV_to_vec(HEV* vertex);
static Eigen::Vector3d HEV_to_vec(HEV* v1, HEV* v2);
static Eigen::Vector3d calc_normal(HEF* face);
static double calc_area(HEF* face);
static Normal calc_vertex_normal(HEV *vertex);
...
static bool build_HE(Mesh_Data *mesh, vector<HEV*> *hevs, vector<HEF*> *hefs) {
...
static double calc_area(Eigen::Vector3d normal) {
  return normal.norm();
}
